...
import { fetchWithRetry, getStarknetAccount } from "../utils/index.ts";
import { ERC20Token } from "../utils/ERC20Token.ts";
...
export class WalletProvider {
  ...
  async getTokenUsdValues(): Promise<CoingeckoPrices> {
    ...
    const cachedValues = await this.runtime.cacheManager.get<CoingeckoPrices>(
      cacheKey
    );
    if (cachedValues) {
  ...
  get: async (
    ...
    const rows = Object.entries(PORTFOLIO_TOKENS)
      .map(([symbol, token]) => {
        ...
        const decimalBalance = Number(rawBalance) / 10 ** token.decimals; // Fix: Use exponentiation operator instead of Math.pow
